// Package rolloutmanager executes the four-phase oper8 rollout loop.
//
...
//   - Python used ThreadPoolExecutor + sleep-poll. Go uses dag.Runner which
//     already runs goroutines with context.Context cancellation.
//   - Components are retrieved from node.Data() — operators store them via
//     node.SetData(comp) in SetupComponents.
//   - Disabled components are not a first-class feature; operators control
//     which components appear in the graph via SetupComponents.
...
// concurrency=0 runs the DAG serially (default; good for tests and dry-runs).
...
func (rm *RolloutManager) Rollout(ctx context.Context) *dag.CompletionState {
	...
	///////////////////////////////////////////////////////////////////////////
	deployState := rm.buildDeployRunner(ctx, components).Run(ctx)
	...
	if !phase1Failed {
		verifyState = rm.buildVerifyRunner(ctx, components, deployState).Run(ctx)
	} else {
	...
	for _, n := range append(deployedAll, verifyState.Unverified...) {
	...
	return &dag.CompletionState{
		...
		Failed:     collectNodes(failedSet, rm.sess),
...
func (rm *RolloutManager) buildDeployRunner(ctx context.Context, comps map[string]component.Component) *dag.Runner {
	runner := dag.NewRunner(rm.sess.Graph, rm.runnerOpts()...)
	for _, n := range rm.sess.Graph.Nodes() {
		...
		n.SetFunc(func() error {
			if err := c.Setup(cx, rm.sess); err != nil {
				return &dag.HaltError{Fatal: true, Cause: err}
			}
			if err := c.Deploy(cx, rm.sess); err != nil {
				return &dag.HaltError{Fatal: true, Cause: err}
			}
			return nil
		})
...
func (rm *RolloutManager) buildVerifyRunner(ctx context.Context, comps map[string]component.Component, deployState *dag.CompletionState) *dag.Runner {
	...
	for _, n := range rm.sess.Graph.Nodes() {
		...
		c, cx := comp, ctx
		n.SetFunc(func() error {
			if !c.Verify(cx, rm.sess) {
				return &dag.HaltError{Fatal: false, Cause: fmt.Errorf("component %q not yet verified", c.Name())}
			}
			return nil
		})
	}
...
func (rm *RolloutManager) runnerOpts() []dag.RunnerOption {
...
func (rm *RolloutManager) componentsByName() map[string]component.Component {
	...
	for _, n := range rm.sess.Graph.Nodes() {
		if c, ok := n.Data().(component.Component); ok {
			out[n.Name()] = c
		}
	}
	return out
...
func collectNodes(nameSet map[string]bool, sess *session.Session) []*dag.Node {
	var out []*dag.Node
